...
export async function getHireDashboardData(userId: string) {
  ...
  const { data: journey } = await supabase
    .from('journeys')
    .select('id, status, current_week, risk_score, risk_reasons, friction_points, start_date, employee:profiles!employee_id(id, full_name, avatar_url), manager:profiles!manager_id(id, full_name, avatar_url), template:journey_templates!template_id(id, name)')
  ...
  const [tasksRes, checkInsRes, resourcesRes] = await Promise.all([
    ...
    supabase
      .from('check_ins')
      .select('id, type, scheduled_date, completed_date, notes')
      .eq('journey_id', journey.id)
