...
export default function JourneyPreview({ onClose }: { onClose: () => void }) {
  ...
  // Check if a template was saved (look for tool outputs with success)
  const templateSaved = messages.some((msg) =>
    msg.parts.some((part) => {
      if (!isToolPart(part)) return false
      return (
        part.toolName === 'saveJourneyTemplate' &&
    ...
    <div className="modal-overlay open" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: '800px', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <h2 style={{ fontFamily: "'Outfit', sans-serif", marginBottom: '4px' }}>
          <i className="fa-solid fa-route" style={{ marginRight: '8px', background: 'var(--grad)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}></i>
          AI Journey Generator
        </h2>
        <p style={{ color: 'var(--text3)', fontSize: '0.85rem', marginBottom: '16px' }}>
          Describe the role and the AI will generate a complete 90-day onboarding journey.
        </p>
              ...
              <div className="chat-bubble">
                {msg.parts.map((part, i) => {
                  ...
                  if (isToolPart(part)) {
                    ...
                    if (toolName === 'saveJourneyTemplate' && state === 'output-available') {
                    ...
                    if (state === 'input-available' || state === 'input-streaming') {
                      return (
                        <div key={i} style={{ fontSize: '0.8rem', color: 'var(--text3)', fontStyle: 'italic', marginTop: '4px' }}>
                          {toolName === 'getExistingTemplates' && 'Checking existing templates...'}
                          {toolName === 'getOrgResources' && 'Looking up organization resources...'}
                          {toolName === 'saveJourneyTemplate' && 'Saving journey template...'}
                        </div>
                      )
                    }
                    return null
